...
def calculate_position():
    ...
    while True:
        ...
        x = encoder_average * math.cos(math.radians(display_roll))
        y = encoder_average * math.sin(math.radians(display_roll))
        
        # Pack as 3 floats (x, y, angle) - 12 bytes total
        # Using little-endian format
        packet = struct.pack('<fff', x, y, display_roll)
        ...
        print(f"x: {x}, y: {y}, angle: {display_roll}")
        time.sleep(0.1)
calculate_position()
